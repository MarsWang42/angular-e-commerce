...
function ProductService($stamplay, $q, $http) {
  ...
  function all() {
    ...
    var products = $stamplay.Object('products');
    products.get()
      .then(function(res) {
        def.resolve(res);
      });
  ...
  function get(id) {
    ...
    var product = $stamplay.Object('products');
  ...
  function update(id, data) {
    ...
    // instanticate a new product model from the stamplay js sdk
    var product = new Stamplay.Object('products').Model;
    product.fetch(id)
      .then(function() {
        // loop over the fields in data and update the product
  ...
  function createPicture(files) {
    ...
    angular.forEach(files, function(file) {
      ...
      $http({
        method: 'POST',
        url: 'https://marsapp.stamplayapp.com/api/cobject/v1/pictures',
        data: fd,
  ...
  function getCategories() {
      ...
      // instanticate a new product collection from the stamplay js sdk
      var products = $stamplay.Object('categories');
      products.get()
          .then(function(res) {
              def.resolve(res);
          });
